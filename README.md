...
│     ├─ media/          ← 图床客户端 + 连接配置
│     └─ api/            ← 各路由（auth/cards/media/fx/funds/options/dashboard/system）
└─ webside/              ← Vue 前端
...
日常做法往往是「先换一笔日元放在日本的账户里，再陆续用它买卡」。这时一张卡的真实人民
币成本取决于**买它的钱是哪一批换进来的**，而不是买卡那天的市场牌价——同一天买的两张
卡，如果吃的是不同批次的钱，成本就不一样。

- **注资**：每次换汇进池记一笔，带自己那天的汇率（自动取牌价，或手填真实换汇价）。
- **扣款**：卡片上打开「从资金池扣除」后，该卡的**日元**购入价与国际运费会在池里生成对应
  扣款；与卡无关的池内支出（手续费等）可在资金池页面手工记。
...
- 分摊是**全量派生数据**：改注资、改金额、删卡之后整体重算（`funds.rebuild()`），结果
  回写到卡片行上（`pool_purchase_cny` / `pool_intl_cny` / `pool_fx_rate`），列表与统计
  直接取用，不必逐行查明细。
- 走资金池的卡，采购侧成本**不再使用** `purchase_fx_rate`；出售侧不受影响（卖卡收的是
  人民币，仍走出售日汇率）。
